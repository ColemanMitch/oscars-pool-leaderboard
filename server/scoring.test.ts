...
describe("computeLeaderboard", () => {
  ...
  it("includes category breakdown per participant", () => {
    ...
    expect(bestActor.winner).toEqual(["Actor A"]);
  ...
  // Tie support tests
  it("awards points for both tied winners", () => {
    const rows = [["ts", "a@b.com", "Alice", "60 points", "30 points", "", ""]];
    const result = computeLeaderboard(rows, columns, { "Best Actor": ["Actor A", "Actor B"] });
    // Alice gets 60 (Actor A) + 30 (Actor B) = 90
    expect(result[0].totalScore).toBe(90);
  });

  it("awards points only for the tied winner the participant picked", () => {
    const rows = [["ts", "a@b.com", "Alice", "90 points", "", "", ""]];
    const result = computeLeaderboard(rows, columns, { "Best Actor": ["Actor A", "Actor B"] });
    // Alice only picked Actor A (90), gets 90 for that, 0 for Actor B
    expect(result[0].totalScore).toBe(90);
  });

  it("stores tied winners as array in category breakdown", () => {
    const rows = [["ts", "a@b.com", "Alice", "60 points", "30 points", "", ""]];
    const result = computeLeaderboard(rows, columns, { "Best Actor": ["Actor A", "Actor B"] });
    const bestActor = result[0].categories.find((c) => c.category === "Best Actor")!;
    expect(bestActor.winner).toEqual(["Actor A", "Actor B"]);
    expect(bestActor.pointsEarned).toBe(90);
  });
});
